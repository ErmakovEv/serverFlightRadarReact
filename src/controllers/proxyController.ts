...
class FlightRadarApiController {
  async getFlights(req: express.Request, res: express.Response) {
    ...
    await https.get(
      `https://data-live.flightradar24.com/zones/fcgi/feed.js?bounds=${bounds}&faa=1&satellite=1&mlat=1&flarm=1&adsb=1&gnd=0&air=1&vehicles=1&estimated=1&maxage=14400`,
      responseHandler(res)
    );
  }
  ...
  async getShedule(req: express.Request, res: express.Response) {
    const { id } = req.params;
    await https.get(
      `https://api.flightradar24.com/common/v1/airport.json?code=${id}`,
      responseHandler(res)
    );
  }
}
